    ...
    <div class="signature-section">
      ...
      <div class="signature-container">
        ...
        <div class="signature-actions" v-if="signatureUrl">
          <el-button type="primary" size="small" @click="downloadSignature">下载签名</el-button>
          <el-button type="danger" size="small" @click="deleteSignature">删除签名</el-button>
        </div>
      </div>
    </div>
...
const loadDiary = async () => {
  try {
    ...
    if (selectedWeek.value !== 17) {
      ...
      diaryDate.value = found ? found.diaryDate || '' : ''
    } else {
      // 总结周，需要同时处理成果和实践内容
...
const loadSignature = async () => {
  try {
    const res = await axios.get(`/student/signature/${props.userName}`)
    if (res.data?.data?.signatureUrl) {
      signatureUrl.value = res.data.data.signatureUrl
    }
  } catch (error) {
    console.error('加载电子签名失败:', error)
  }
}
...
const downloadSignature = () => {
  if (signatureUrl.value) {
    ...
    link.download = `signature_${props.userName}.png`
...
const deleteSignature = async () => {
  try {
    await axios.delete(`/student/signature/${props.userName}`)
    signatureUrl.value = ''
    ElMessage.success('电子签名删除成功')
  } catch (error) {
